...
public class SemAnalyzer extends VisitorAdaptor {
	...
	public void visit(ConstDeclList1 ConstDeclList1) {
		...
		report_info("Obilazak konstante " + ConstDeclList1.getI1() + " " + currType.getKind(), ConstDeclList1);
	}
	...
	/*
	@Override
	public void visit(Final fin) {
		isFinal = true;
	}
	
	@Override
	public void visit(VarDeclList_modif varModif) {
		isFinal = false;
	}*/
	...
	public void visit(VarItem_ident varDecl) {
		Obj var = null;
		if(isFinal == true) {
			report_error("Final promenljive nije dozvoljen: ", varDecl);
		}
		if(currMethod != null) {
		...
		report_info("Obilazak promenljive " + varDecl.getI1() + " " + currType.getKind(), varDecl);
	}
	...
	public void visit(VarItem_array varDeclArray) {
		...
		report_info("Obilazak niza " + varDeclArray.getI1() + " " + currType.getKind(), varDeclArray);
	}
	...
	public void visit(MethodSignature_void msIdent) {
		msIdent.obj = currMethod = Tab.insert(Obj.Meth, msIdent.getI1(), Tab.noType);
		Tab.openScope();
	...
	public void visit(MethodSignature_type msIdent) {
		msIdent.obj = currMethod = Tab.insert(Obj.Meth, msIdent.getI2(), currType);
		Tab.openScope();
	}
	...
	@Override
	public void visit(DesignatorStatement_modif designModif) {
		Struct left = designModif.getDesignator().obj.getType();
		Struct right = designModif.getDesignator1().obj.getType();
		
		if(!left.equals(Tab.intType)) {
			report_error("Promenljiva sa leve strane mora biti tipa int ", designModif);
		}
		if(!(right.getKind() == Struct.Array) || !right.getElemType().equals(Tab.intType)) {
			report_error("Mora biti niz i to tipa int ", designModif);
		}
	}
	...
	public void visit(Designator_node designNode) {
		...
		if(designNode.getExpr().struct.equals(Tab.intType)) {
			if(desigArr != Tab.noObj) {
				...
				if(desigArr.getFpPos() == 2) {
					designNode.obj.setFpPos(3);
				}
			}
			else {
	...
	public void visit(Factor1Square_node factPars) {
		...
		if(getKind != Obj.Meth) {
			...
			for(Obj symbol : factPars.getDesignator().obj.getLocalSymbols()) {
				//&& symbol.getLevel() == 1 
				if(i < factPars.getDesignator().obj.getLevel()) {
	...
	public void visit(Choose_ds_p stmtMeth) {
		...
		if(getKind != Obj.Meth) {
			...
			for(Obj symbol : stmtMeth.getDesignator().obj.getLocalSymbols()) {
				//&& symbol.getLevel() == 1 
				if(i < stmtMeth.getDesignator().obj.getLevel()) {
					paramList.add(symbol.getType());
					i++;
				}
				else {
