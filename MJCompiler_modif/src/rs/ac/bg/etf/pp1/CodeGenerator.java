...
public class CodeGenerator extends VisitorAdaptor {
	...
	public void visit(DesignatorStatement_modif designModif) {
		Code.load(designModif.getDesignator().obj); // d0 - n
		Code.load(designModif.getDesignator1().obj); // d1 - 1
		
		Code.put(Code.enter);
		Code.put(2); // d0, d1
		Code.put(4); // i - 2, max - 3
		
		Code.loadConst(0);
		Code.put(Code.store_3);
		
		Code.put(Code.load_1); // d1
		Code.put(Code.arraylength); // len 
		Code.loadConst(0); // len 0
		Code.put(Code.store_2); // len  |  i = 0
		Code.put(Code.load_2); // len i | len > i, i > len skaci 
		...
		int pc6 = Code.pc - 2;
		Code.put(Code.load_1); //d1 
		Code.put(Code.load_2); //d1 i
		Code.put(Code.aload); //d1[i]
		Code.put(Code.load_3);//d[i] max
		
		
		Code.putFalseJump(Code.gt, 0); // d[i] > max ne skaci
		int pc7 = Code.pc - 2;
		
		Code.put(Code.load_1); //d1 
		Code.put(Code.load_2); //d1 i
		Code.put(Code.aload); //d1[i]
		Code.put(Code.store_3); // - | max = d1[i]
		...
		Code.put(Code.load_3); // max
		Code.store(designModif.getDesignator().obj);
		Code.put(Code.exit);
	...
	public void visit(Choose_ds_a designAssign) {
		//kao nalaz se argumenti na steku -> niz - n, index - 1, vrednost - 2
		if(designAssign.getDesignator().obj.getFpPos() == 3) {
			//Code.put(Code.trap);
			Code.put(Code.enter);
			Code.put(3);
			Code.put(4); // i2 - 3
			
			Code.put(Code.load_n);// 1 arr
			Code.put(Code.arraylength); //len
			Code.loadConst(2);
			Code.put(Code.div);
			Code.put(Code.store_3);// -
			...
			Code.put(Code.load_3); // index len
			Code.put(Code.add); // index + len
			Code.put(Code.store_3);// i2 = index + len
			Code.put(Code.load_n);// arr
			Code.put(Code.load_3); // arr i2
			Code.put(Code.aload); // arr[i2]
			Code.loadConst(0);// arr[i2] 0
			
			Code.putFalseJump(Code.ne, 0); //  ne skaci
			int pc7 = Code.pc - 2;
			Code.put(Code.trap);
			...
			Code.put(Code.load_n);// arr
			Code.put(Code.load_1); // arr index
			Code.put(Code.load_2); // arr index val
